...
type Payable = {
   ...
   * INVOICE SUPPLIER
   *
   * Untuk Purchase TEMPO, field ini adalah
   * sumber utama nomor invoice supplier.
   */
...
type Purchase = {
   ...
   * Fallback data lama.
   *
   * Untuk Purchase TEMPO, invoice utama tetap
...
export default function PurchasePaymentPage() {
  ...
  async function loadPurchase() {
    ...
    try {
       ...
       * ===================================================
       *
       * Jika payable tersedia:
       * gunakan outstanding dari PurchasePayable.
       *
       * Jika belum tersedia:
       * fallback ke total Purchase.
       */
      const payable =
        payload?.payable;
  ...
  /**
   * Untuk Purchase TEMPO:
   *
   * PRIORITAS:
   * 1. purchase.payable.invoiceNumber
   * 2. purchase.invoiceNumber
   *
   * invoiceNumber pada payable adalah invoice supplier
   * yang dimasukkan ketika Barang Masuk.
   */
  const supplierInvoiceNumber =
    purchase?.payable?.invoiceNumber?.trim() ||
    purchase?.invoiceNumber?.trim() ||
    "";

  /**
   * Tanggal invoice juga diprioritaskan dari payable.
   */
  ...
  /**
   * Jatuh tempo diprioritaskan dari PurchasePayable.
   */
  const supplierDueDate =
  ...
  async function submitPayment() {
    ...
    /*
     * Client validation hanya sebagai UX.
     *
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm leading-6 text-purple-900">
                    Invoice supplier pada halaman
                    ini mengambil data dari
                    Purchase Payable yang dibuat
                    ketika Goods Receipt / Barang
                    Masuk.
                  </p>
