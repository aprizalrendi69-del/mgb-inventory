...
function normalizeCategory(value: any): string {
  ...
  if (!n) return "";
  if (n.includes("VEGET") || n.includes("SAYUR")) return "VEGETABLES";
  if (n.includes("FRUIT") || n.includes("BUAH")) return "FRUITS";
  if (n.includes("GROC")) return "GROCERIES";
  if (n.includes("PASTR") || n.includes("BAKERY")) return "PASTRY";
  if (n.includes("FROZEN")) return "FROZEN FOOD";
  if (n.includes("SEAFOOD") || n.includes("SEA FOOD")) return "SEAFOOD";
  if (n.includes("CHICKEN") || n.includes("POULTRY") || n.includes("AYAM")) return "CHICKEN & POULTRY";
  if (n.includes("WIP") || n.includes("SOUP")) return "WIP & SOUP";
  if (n.includes("BEVER") || n.includes("MINUM")) return "BEVERAGE";
  if (n.includes("MEAT") || n.includes("DAGING")) return "MEATS";
  if (n.includes("KITCHEN") || n.includes("FUEL") || n.includes("GAS")) return "KITCHEN FUEL";
  if (n.includes("STAFF") || n.includes("KARYAWAN")) return "STAFF MEALS";
  if (n.includes("CLEAN")) return "CLEANING SUPPLY";
  if (n.includes("GUEST")) return "GUEST SUPPLY";
  if (n.includes("PACK")) return "PACKAGING";
  return n;
}
...
function normalizeCategoryRows(rows: any[] = []) {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      ...
      for (const key of ["opening", "purchase", "received", "ending", "waste", "cost", "sales", "netSales"]) {
        existing[key] = Number(existing[key] ?? 0) + Number(row?.[key] ?? 0);
      }
      existing.ratioFoodCost = Number(existing.sales ?? existing.netSales ?? 0) > 0
        ? (Number(existing.cost ?? 0) / Number(existing.sales ?? existing.netSales ?? 0)) * 100
        : 0;
      existing.foodCost = existing.ratioFoodCost;
      existing.ratioToSales = existing.ratioFoodCost;
    }
  }
  ...
  return COST_CATEGORIES.map((category) =>
    ...
    Array.from(map.entries())
      .filter(([category]) => !(COST_CATEGORIES as readonly string[]).includes(category))
      .map(([, row]) => row)
  );
}
...
function groupRows(rows: any[]) {
  rows = normalizeCategoryRows(rows || []);
  const foodCost: any[] = [];
  const others: any[] = [];
...
export default function CostControlPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        // Resolve role FIRST so an outlet admin never renders
        // the global outlet selector while /api/outlet loads.
        ...
        if (outletAdmin && ownOutletId > 0) {
          setOutletId(String(ownOutletId));
        }
        ...
        const outletJson = await outletResponse.json();
        const outletData = Array.isArray(outletJson?.data)
          ? outletJson.data
          : [];
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-4 md:p-6 lg:p-8">
      ...
      <section className="relative mb-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_18px_55px_rgba(15,23,42,0.07)]">
        <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-emerald-600 via-teal-500 to-emerald-300" />
        <div className="border-b border-slate-100 px-5 py-5 md:px-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="mb-1 flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.18em] text-emerald-600">
                <ShieldCheck size={14} />
                Report Control
              </div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Pilih periode laporan dan outlet yang ingin dianalisis.
              </p>
            </div>
            {isOutletAdmin && (
              <div className="inline-flex w-fit items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">
                <LockKeyhole size={13} />
                Outlet terkunci ke akun Anda
              </div>
            )}
          </div>
        </div>

        <div className="grid grid-cols-1 gap-4 px-5 py-5 md:grid-cols-2 md:px-6 lg:grid-cols-4">
          <div>
            ...
            <select
              value={outletId}
              disabled={!userReady || isOutletAdmin}
              onChange={(e) => {
                if (!isOutletAdmin) {
              ...
              className={`
                ...
                ${
                  isOutletAdmin
                    ? "cursor-not-allowed border-emerald-200 bg-emerald-50/70 text-emerald-800"
                    : ""
                }
              `}
            >
      ...
      {!userReady && (
        <div className="mb-6 flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 text-sm font-semibold text-slate-600 shadow-sm">
          <RefreshCw size={18} className="animate-spin text-emerald-600" />
          Memverifikasi akses Cost Control...
        </div>
      )}

      {loading && userReady && (
        <div className="mb-6 flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-semibold text-emerald-700">
          <RefreshCw
            size={18}
              ...
              (outlet: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* MONTHLY CATEGORY RECAP */}

                    <div className="mb-8">
                      <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                          <h3 className="text-base font-bold text-slate-900">Hasil Stock Opname — Recap Bulanan</h3>
                          <p className="mt-0.5 text-xs text-slate-500">Kategori dihitung langsung dari data Stock Opname dan transaksi Cost Control.</p>
                        </div>
                        <span className="text-[10px] font-bold uppercase tracking-wide text-emerald-600">VEGETABLES • MEATS • DLL</span>
                      </div>
                      <CostGroupTable
                        title="MONTHLY COST CONTROL"
                        rows={normalizeCategoryRows(outlet.monthly?.rows || [])}
                        outlet={outlet}
                        total={getTotalStatus(normalizeCategoryRows(outlet.monthly?.rows || []), outlet)}
                      />
                    </div>
