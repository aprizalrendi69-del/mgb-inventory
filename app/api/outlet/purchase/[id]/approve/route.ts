...
TEMPO
    -> TIDAK mengurangi Petty Cash
    -> TIDAK membuat PurchasePayable saat APPROVE
    -> TIDAK membuat Payment
    -> PurchasePayable dibuat saat GOODS RECEIPT
    -> Invoice Supplier diinput manual saat GOODS RECEIPT
  ...
  try {
      ...
      await prisma.$transaction(
        async (tx) => {
          ...
          const purchase =
            await tx.outletPurchase.findUnique({
              ...
              include: {
                ...
                /*
                PENTING:
                payable tetap boleh di-load untuk
                pengecekan legacy.

                Tetapi APPROVE TIDAK AKAN
                membuat payable baru.
                */
                payable: true,
          ...
          */
          ...
          /*
          PENTING:
          APPROVE TIDAK MEMBUAT PAYABLE.

          Variable ini hanya dipertahankan
          untuk struktur response agar tidak
          merusak consumer API yang sudah ada.
          */
          ...
          */
          ...
          - Invoice supplier diinput saat Goods Receipt
          ==================================================
          */
          ...
          if (
            ...
            ------------------------------------------------
            PENTING:
            JANGAN BUAT PAYABLE DI SINI.
            ------------------------------------------------

            Sebelumnya ada:

              tx.purchasePayable.create({
                ...
                invoiceNumber:
                  `INV-${purchase.number}`,
                ...
            ...
            Invoice supplier belum diketahui saat
            Purchase Outlet diapprove.
            ...
            */
            ...
            Jika payable sudah ada karena transaksi lama,
            kita TIDAK menghapus atau mengubahnya.

            Approval baru tidak membuat payable baru.
            ------------------------------------------------
            */

            // Tidak ada operasi database di sini.
            // Tidak create payable.
            // Tidak update payable.
            // Tidak create payment.
            // Tidak update petty cash.
          }
          ...
          */
          ...
          return {
            ...
            /*
            Untuk TEMPO:
            selalu null pada proses APPROVE baru.

            Payable baru akan muncul saat Goods Receipt.
            */
            payable,
          };
        },
    ...
    */
    ...
    if (
      ...
      message =
        "Purchase Outlet berhasil diapprove. Pembayaran TEMPO belum dibuat sebagai hutang. Purchase Payable akan dibuat saat Goods Receipt setelah No. Invoice Supplier dimasukkan.";
    }
